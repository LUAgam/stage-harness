...
# 先检测元问题（用户在讨论 harness 工具/流程本身，而非项目需求）
IS_META_QUESTION=false
if echo "$PROMPT_TEXT" | grep -qiP "(为啥|为什么|怎么).*(错误|报错|失败|异常)"; then
  IS_META_QUESTION=true
elif echo "$PROMPT_TEXT" | grep -qiP "(以上|上面|刚才|之前).*(执行|输出|结果|报错)"; then
  IS_META_QUESTION=true
elif echo "$PROMPT_TEXT" | grep -qiE "harnessctl|stage-harness|stage.gate|harness.*流程|harness.*问题"; then
  IS_META_QUESTION=true
elif echo "$PROMPT_TEXT" | grep -qiP "(流程|工具|命令|插件).*(问题|bug|调整|改进|修复)"; then
  IS_META_QUESTION=true
elif echo "$PROMPT_TEXT" | grep -qiP "总结.*问题|梳理.*问题|分析.*调整"; then
  IS_META_QUESTION=true
fi

IS_CORRECTION=false
if [[ "$IS_META_QUESTION" == "false" ]]; then
...
if [[ -n "$FEEDBACK_CANDIDATE_TYPE" ]]; then
  if [[ -n "$AUTO_FB_ID" ]]; then
    ...
    REMINDER+="\n        （此命令完成：evidence-pack + council-triage init。返回 council_dispatch_required 信号）"
    REMINDER+="\n     2. 并行调度 feedback_triage_council 6 agent 评审（使用 Agent 工具，每个 agent 通过 write-vote 提交）"
    REMINDER+="\n     3. harnessctl feedback aggregate-triage --epic-id ${ACTIVE_EPIC_ID} --feedback-id ${AUTO_FB_ID}"
    REMINDER+="\n     4. harnessctl feedback continue --epic-id ${ACTIVE_EPIC_ID} --feedback-id ${AUTO_FB_ID} --execute"
    REMINDER+="\n        （根据 verdict 自动执行后续：reopen/close/create-task）"
    REMINDER+="\n"
    REMINDER+="\n     ⛔ 硬性规则："
