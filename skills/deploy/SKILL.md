...
- `name`：子项目标识（唯一）
- `path`：相对于 PROJECT_ROOT 的路径
- `deploy_command`：部署命令（skip 时为空）
- `deploy_type`：部署类型（`custom-script` / `docker-compose` / `node` / `k8s` / `makefile` / `cloud-platform`）
- `skip`：`true` 表示不需要部署
- `skip_reason`：跳过原因（可选）
...
扫描完成后，将结果写入 `project-profile.yaml` 的 `sub_projects` 字段持久化，并输出扫描摘要（格式：`✔ name (path) → command [type]` 或 `⊘ name (path) → 跳过（原因）`）。
