...
**P3 — 凭证最大化复用（按维度分类存储）**
用户提供过登录信息后，按维度分别存储以便复用：
- **原始凭证**（账号密码 / 手机号等用户提供的原始登录信息）→ 写入 `session-state.json` 的 `credentials.raw` 字段，供后续 case 的 B3 步骤复用，避免重复询问用户
- **API token**（JWT / API Key / session cookie）→ 写入 `session-state.json` 的 `credentials.api_token` 字段，供 API 维度 case 复用
- **浏览器登录态**（Playwright storage state）→ 通过 `context.storage_state(path=...)` 保存为文件，将路径写入 `session-state.json` 的 `credentials.browser_storage_state` 字段，供 UI / API+UI 维度 case 复用

后续 case 的复用优先级：已存储的同类凭证 > 用原始凭证重新登录 > 再次询问用户。
仅在以下情况再次询问：
- 所有已存储的凭证均过期或被撤销
- 需要不同角色/租户身份
- 需要重新走登录流程本身的验证
...
3. 若无可复用登录态或已过期 → 在浏览器中导航到登录页，通过以下方式完成登录：
   a. 分析登录页面的表单结构（输入框、按钮），截图记录
   b. 若 `session-state.json` 的 `credentials.raw` 已有原始凭证 → 用 Playwright 在前端页面自动填写表单并提交
...
2. 根据 case 维度确定所需凭证类型：
   - **API 维度** → 需要 `credentials.api_token`（JWT / API Key / session cookie）；可通过后端 API 登录获取
   - **UI / API+UI 维度** → 需要 `credentials.browser_storage_state`（Playwright storage state）；若无则需要原始凭证在前端登录页操作（参见 P14），**禁止通过后端 API 获取 token 注入浏览器**
3. 检查 `session-state.json` 是否已有对应类型的可复用凭证
4. 不可复用时通过 `AskUserQuestion` 向用户索要**原始凭证**（账号密码 / 手机号等），而非 token
5. 获取原始凭证后：
   - API case → 可通过后端 API 获取 token，写入 `credentials.api_token`
   - UI / API+UI case → 必须在 Playwright 浏览器前端登录页完成登录（参见 P14），保存 storage state 写入 `credentials.browser_storage_state`
