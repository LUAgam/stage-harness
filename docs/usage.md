...
4. 输出推荐下一步：
   - 自动推进：`/harness:auto <epic-id>`
   - 手动推进：`/harness:clarify <epic-id>`
...
**CLARIFY 跨阶段 Bash 拦截**：`hooks/scripts/pre-tool-use.sh` 会在 Bash 命令**明确指向仍处于 `CLARIFY` 的 epic** 时，拒绝执行 `/harness:spec|plan|work|review|done|patch|auto|bridge|fix` 及等价的 `harness:` / `stage-harness:harness-*` 写法，避免并行子任务把该 epic 拖到后续阶段；其它 epic 的正常推进、以及单纯输出 slash 文本的命令不应被误拦。
