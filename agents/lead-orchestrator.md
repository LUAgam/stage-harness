...
- **Semantic closure check**: When marking a SCN as `covered` via a mapped REQ/CHK, verify that the mapped target actually resolves the risk described in `why_it_matters`. A mapping that matches by topic or keyword but does not address the specific risk (e.g., a type mapping that covers the type name but ignores precision/range loss described in the SCN) is NOT closed — downgrade its status to `needs_decision` and emit a DEC or UNK for the unresolved aspect.
