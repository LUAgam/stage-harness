...
### Step 0.5 — Source Material Loading（自适应，零开销）

在 domain-scout 完成后、进入 Step 1 之前，Lead 检查 `.harness/features/<epic-id>/requirement-index.json`：

- **`input_density: "rich"`**：读取同目录 `source-materials.md`，提取用户原始需求的完整文本。后续 Step 1 的 Q&A 分析、Step 2 的并行 agent 派发均须将 `source-materials.md` 路径作为显式输入。
- **`input_density: "minimal"` 或文件不存在**：跳过，无额外动作。仅使用 epic description。

本步骤确保富文档需求的原始精度在整个 CLARIFY 流程中可追溯，同时对简单需求零开销。
