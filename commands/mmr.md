...
说明：Claude Code 插件内这些 agent 仍使用 `model: inherit`，避免插件在未配置外部 provider 时加载失败。表中的“目标模型”是路由意图与复审视角；如需真实强制路由到 `gpt-5.5`、`google/gemini-3.1-pro-preview`、`claude-opus-4.6-thinking`，需要宿主或外部模型网关支持这些模型标识。
